...
# filter_backends:-A list of filter backend classes hat should be used for filtering the queryset.Defaults to the same value as the DEFAULT_FILTER_BACKENDS setting.
# Methods:-
# get_queryset(self):It returns the queryset that should be used for list views,and that should be used as the base for lookups in detail views.Defaults to returning the queryset specified b the queryset attribute.
...
# 
# get_serialize_context(self):-It returns a dictionary containing any extra context that should be supplied to the serializer.Defaults to including 'request','view' and 'format' keys.
...
# filter_queryset(self,queryset)-Given a queryset,filter it with whicheverfilter backends are in use,returning a new queryset.

#Mixins:-One of the big wins of using class -based views is that it allows us to easily compose reusable bits of behaviour. 
...
# If the queryset is populated ,this returns a 200 OK response.The response data may optionally be paginated
...
#          return self.list(request,*kargs,**kwargs)

#CreateModelMixin:-It provides a create(request,*args,**kwargs) method,that implements creating nd saving a new model instance.
# If an object is created this returns a 201 Created response,with a serialized representation of the object as the body of the response.If the representation contains a key named url,then the Location header of the response will be populated with that value.
# If the request data provided for creating the object was invalid a 400 Bad request response will be returned,with error details as the body of the response.
# ex:-
...
# RetriveModelMixin:-It provides retrieve(request,*args,**kwargs) method,that implements returning an existing model instance in a response.
# if an object can be retrieved this returns a 200 OK response,will a serialized representation of the object as the body of the response.Otherwise it will return a 404 Not Found.
# ex:-
# from rest_framework.mixins import RetrieveModel
# from rest_framework.generics import GenericAPIView
...
# UpdateModelMixin:-
# It provides a update(requets,*args,**kwargs) method,that implements updating and saving an existing model instance.
...
#Ex:-
# from rest_framework.mixins import DestroyModelMixins
# from rest_framework.generics import GenericAPIView 
